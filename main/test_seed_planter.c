...
static void test_seed_planter_task(void *pvParameters)
{
    ...
    for (;;)
    {
        if (xQueueReceive(seed_planter_handle, &seed_planter_data, portMAX_DELAY) == pdPASS)
        {
            ...
            // printf()
            printf("/*cutter_desired_speed,%f,speed_cutter,%f,dispenser_desired_speed,%f,speed_dispenser,%f,state,%d*/\r\n", seed_planter_data.motor_left_desired_speed, seed_planter_data.motor_left_real_pulses, seed_planter_data.motor_right_desired_speed, seed_planter_data.motor_right_real_pulses, seed_planter_data.state);
#endif
        }
        else
        {
